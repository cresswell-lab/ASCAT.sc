run_sc_sequencing_smoothing <- function(tumour_bams,
                            res=NULL,
                            ...
                            sc_exclude_badbins=FALSE)
{
    ...
    if(multipcf)
    {
        ...
        res$allTracks.smoothed <- mclapply(lapply(names(res$allTracks), function(cell.name) res$allTracks[[cell.name]][[2]]), function(cell.nlCTS)
            {
                lCTS.smoothed = smooth_tracks_k_bins(cell.nlCTS,
    ...
    else
    {
        if(!any("allTracks.processed"%in%names(res)) | res$binsize!=binsize)
        {
            ...
            {   
                # Smoothing out the tracks by averaging over a surrounding window
                getTrackForAll(bamfile=NULL,
                            window=NULL,
                            lCT=res$allTracks.smoothed[[x]],
                            lSe=res$nlSe,
                            lGCT=res$nlGCT,
                            lNormals=res$lNormals,
                            allchr=allchr,
